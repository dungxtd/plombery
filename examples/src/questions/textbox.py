...
from config.selectors import get_textbox_selector
...
class SAQuestion(BaseQuestion):
    ...
    """
    # Define constants
    _SAQ_SELECTOR_NAME = get_textbox_selector()
    ...
    def get_info(self) -> Optional[bool]:
        ...
        elements = self._QUESTION_ELEMENT.find_elements(By.CSS_SELECTOR, self._SAQ_SELECTOR_NAME)
        if not elements:
            _logger.error(f"No answer elements found using selector {self._SAQ_SELECTOR_NAME}")
            return False
        self.set_answer_elements(elements[0])
        return True
