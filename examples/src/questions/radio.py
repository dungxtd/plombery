...
from config.selectors import get_radio_selector, get_radio_other_selector
...
class RadioQuestion(BaseOptionQuestion):
    ...
    # Define constants
    _CSS_SELECTOR_NAME = get_radio_selector()  # Radio buttons
    _OTHER_CSS_SELECTOR = get_radio_other_selector()  # 'Other' Input Field
    ...
    def get_info(self) -> Optional[bool]:
        ...
        container = self._QUESTION_ELEMENT.find_element(By.CSS_SELECTOR, BaseOptionGridQuestion.get_container_class()) \
            if isinstance(self, BaseOptionGridQuestion) else self._QUESTION_ELEMENT
        elements = container.find_elements(By.CSS_SELECTOR, self._CSS_SELECTOR_NAME)
        option_elements, options = [], []
        for element in elements:
            option = element.get_attribute("aria-label")

            # Check if there is an 'Other' option specified
            if option == self._OTHER_OPTION_ARIA_LABEL and \
                ...
                self.set_other_option_element(self._QUESTION_ELEMENT.find_element(By.CSS_SELECTOR, 
                    self._OTHER_CSS_SELECTOR))  # Input field associated with the 'Other' option
